...
METADATA_COMPARING_ALLOWED = config.getboolean('bot', 'metadata_comparing', fallback=True)
HISTORY_ALLOWED = config.getboolean('bot', 'history', fallback=True)
    ...
    def generate_leaderboard_embed(self, track_data, entry_data, instrument):
        ...
        for session in entry_data.get('sessions', []):
            ...
            if not is_solo:
                ...
                session_field_text += f"[Band Score] {band['accuracy']}% {self.format_stars(band['stars'])} {band['scores']['total']}\n"
...
class BotSessionUtils():
    def generate_session_hash(self, user_id, song_name):
...
class BotEmbedUtils():
    def generate_difficulty_bar(self, difficulty, max_blocks=7):
    ...
    def generate_difficulty_string(self, difficulty_data):
        """Generate a formatted difficulty string from the difficulty data."""
        return (
            f"Lead:      {self.generate_difficulty_bar(difficulty_data.get('gr', 0))} "
            f"Bass:      {self.generate_difficulty_bar(difficulty_data.get('ba', 0))} "
            f"Drums:     {self.generate_difficulty_bar(difficulty_data.get('ds', 0))}\n"
            f"Pro Lead:  {self.generate_difficulty_bar(difficulty_data.get('pg', 0))} "
            f"Pro Bass:  {self.generate_difficulty_bar(difficulty_data.get('pb', 0))} "
            f"Vocals:    {self.generate_difficulty_bar(difficulty_data.get('vl', 0))}"
        )
    ...
    def create_track_embeds(self, track_list, title, daily_shortnames={}, chunk_size=10, shop=False, jam_tracks=None):
            ...
            for track in chunk:
                if shop:
                    ...
                    if jam_track:
                        ...
                        difficulty_str = self.generate_difficulty_string(difficulty_data)
                    else:
    ...
    def generate_track_embed(track_data, is_new=False, is_removed=False):
        ...
        difficulties = (
            f"Lead:      {embed_utils.generate_difficulty_bar(guitar_diff)}\n"
            f"Bass:      {embed_utils.generate_difficulty_bar(bass_diff)}\n"
            f"Drums:     {embed_utils.generate_difficulty_bar(drums_diff)}\n"
            f"Vocals:    {embed_utils.generate_difficulty_bar(vocals_diff)}\n"
            f"Pro Lead:  {embed_utils.generate_difficulty_bar(pro_guitar_diff)}\n"
            f"Pro Bass:  {embed_utils.generate_difficulty_bar(pro_bass_diff)}\n"
            f"Pro Drums: {embed_utils.generate_difficulty_bar(pro_drums_diff)}"
        )
        ...
        for value, name in difficulty_comparisons.items():
            if old_track_data['in'].get(value, 0) != new_track_data['in'].get(value, 0):
                embed.add_field(
                    ...
                    value=f"```Old: \"{embed_utils.generate_difficulty_bar(old_track_data['in'].get(value, 0))}\"\nNew: \"{embed_utils.generate_difficulty_bar(new_track_data['in'].get(value, 0))}\"```", 
                    inline=False
                )
        ...
        for key in new_track_data['in'].keys():
            if key not in difficulty_comparisons.keys() and key != '_type':
                embed.add_field(
                    ...
                    value=f"```Found: {embed_utils.generate_difficulty_bar(new_track_data['in'][key])}```", 
                    inline=False
                )
...
class BotMIDIUtils():
    ...
    async def process_chart_url_change(self, old_url, new_url, channel, track_name, song_title, artist_name, album_art_url, last_modified_old, last_modified_new, session_hash):
        # Decrypt old .dat to .midi
        old_midi_file = self.midi_utils.decrypt_dat_file(old_url, session_hash)

        # Decrypt new .dat to .midi
        new_midi_file = self.decrypt_dat_file(new_url, session_hash)
        ...
        if old_midi_file and new_midi_file:
            ...
            if "MIDI comparison completed successfully" in result.stdout:
                ...
                if comparison_images:
                    ...
                    session_utils.delete_session_files(session_hash)
                else:
    ...
    def track_midi_changes(self, json_files, shortname, session_hash):
        ...
        try:
            for idx, json_file in enumerate(json_files):
                ...
                file_content = self.load_json_from_file(file_path)
                if not file_content:
                ...
                if midi_file_url and midi_file_url not in seen_midi_files:
                    ...
                    local_midi_file = self.download_and_archive_midi_file(midi_file_url, shortname)  # Download the .dat file
    ...
    # Helper function to modify the MIDI file for Pro Lead and Pro Bass and Pro Drums
...
class BotDiskUtils():
    def save_known_songs_to_disk(self, songs):
        ...
        recent_files_content = [misc_utils.load_json_from_file(os.path.join(folder_path, f)) for f in json_files[:3]]
    ...
    def load_known_songs_from_disk(self, shortnames:bool = False):
...
class BotMiscUtils():
    ...
    def fuzzy_search_tracks(self, tracks, search_term):
    ...
    def fetch_jam_tracks_file(self):
        try:
            response = requests.get(API_URL)
            data = response.json()
    ...
    async def handle_imacat_search(ctx):
        ...
        embed = embed_utils.generate_track_embed(imacat_data)
        embed.add_field(name="Status", value="Removed from API. This song has never been officially obtainable.", inline=False)
        await ctx.send(embed=embed)

    async def display_track_info(self, ctx, track_data, shop_tracks, daily_shortnames_data):
        embed = embed_utils.generate_track_embed(track_data)
    ...
    async def prompt_user_for_selection(self, ctx, matched_tracks, shop_tracks, daily_shortnames_data, bot):
        ...
        try:
            ...
            await self.display_track_info(ctx, chosen_track, shop_tracks, daily_shortnames_data)
    ...
    def process_daily_tracks(self, tracks, daily_shortnames_data):
        ...
        for track in tracks.values():
            ...
            if shortname in daily_shortnames_data:
                ...
                difficulty_str = embed_utils.generate_difficulty_string(track['track'].get('in', {}))
...
leaderboard_functions = BotLeaderboardUtils()
session_utils = BotSessionUtils()
embed_utils = BotEmbedUtils()
midi_utils = BotMIDIUtils()
disk_utils = BotDiskUtils()
misc_utils = BotMiscUtils()
...
class FestivalInfoBot():
    """Class with all the commands for the bot"""
    def __init__(self) -> None:
        ...
        bot = commands.Bot(
            command_prefix=session_utils.get_prefix,
            intents=intents,
        ...
        async def search(ctx, *, query: str = None):
            ...
            if query.lower() in {"i'm a cat", "im a cat", "imacat"}:
                await misc_utils.handle_imacat_search(ctx)
                return

            # Fetch data from various APIs
            tracks = misc_utils.fetch_available_jam_tracks()
            if not tracks:
                await ctx.send('Could not fetch tracks.')
                return

            daily_shortnames_data = misc_utils.fetch_daily_shortnames()
            shop_tracks = misc_utils.fetch_shop_tracks()

            # Perform fuzzy search
            matched_tracks = misc_utils.fuzzy_search_tracks(tracks, query)
            if not matched_tracks:
                await ctx.send('No tracks found matching your search.')
                return

            if len(matched_tracks) == 1:
                await misc_utils.display_track_info(ctx, matched_tracks[0], shop_tracks, daily_shortnames_data)
            else:
                await misc_utils.prompt_user_for_selection(ctx, matched_tracks, shop_tracks, daily_shortnames_data, bot=bot)
        ...
        async def daily_tracks(ctx):
            ...
            daily_tracks = misc_utils.process_daily_tracks(tracks, daily_shortnames_data)

            if daily_tracks:
                embeds = embed_utils.create_daily_embeds(daily_tracks)
        ...
        async def tracklist(ctx):
            ...
            track_list = embed_utils.prepare_track_list(tracks)
            ...
            embeds = embed_utils.create_track_embeds(track_list, title, daily_shortnames={})
        ...
        async def shop_tracks(ctx):
            ...
            tracks = misc_utils.fetch_shop_tracks()
            jam_tracks = misc_utils.fetch_available_jam_tracks()  # Fetch data from the jam API to cross-reference difficulties

            if not tracks:
                await ctx.send('Could not fetch shop tracks.')
                return

            track_list = embed_utils.prepare_track_list(tracks, shop=True)
            ...
            embeds = embed_utils.create_track_embeds(track_list, title, daily_shortnames={}, shop=True, jam_tracks=jam_tracks)
        ...
        async def generate_path(ctx, songname: str, instrument: str = 'guitar', *args):
            ...
            try:
                ...
                session_hash = session_utils.generate_session_hash(user_id, songname)  # Unique session identifier
                ...
                if songname.lower() in ["i'm a cat", "im a cat", "imacat"]:
                    ...
                    matched_tracks = misc_utils.fuzzy_search_tracks(tracks, songname)
                    if not matched_tracks:
                ...
                display_instrument, instrument_codename = misc_utils.get_instrument_from_query(instrument)  # Get user-friendly instrument name
                ...
                local_midi_file = misc_utils.download_and_archive_midi_file(song_url, short_name)  # Download the .dat file
                ...
                midi_file = midi_utils.decrypt_dat_file(local_midi_file, session_hash)
                if not midi_file:
                ...
                if 'Peripheral' in instrument_codename:
                    modified_midi_file = misc_utils.modify_midi_file(midi_file, instrument, session_hash, short_name)
                ...
                session_utils.delete_session_files(session_hash)  # Clean up session files like MIDI and images
                await thinking_message.delete()
        ...
        async def history(ctx, *, song_name: str = None):
            ...
            user_id = ctx.author.id
            session_hash = session_utils.generate_session_hash(user_id, song_name)
            ...
            matched_tracks = misc_utils.fuzzy_search_tracks(tracks, song_name)
            if not matched_tracks:
            ...
            json_files = misc_utils.fetch_local_history()
            if not json_files:
            ...
            midi_file_changes = midi_utils.track_midi_changes(json_files, shortname, session_hash)
            ...
            for i in range(1, len(midi_file_changes)):
                ...
                await midi_utils.process_chart_url_change(
        ...
        async def bot_stats(ctx):
            ...
            uptime = misc_utils.format_uptime(uptime_seconds)
            
            # Get the last GitHub update and latest commit hash
            latest_commit_hash, last_update = misc_utils.fetch_latest_github_commit_hash()
            
            # Get the current local commit hash
            local_commit_hash = misc_utils.get_local_commit_hash()

            # Check if the local commit hash is up-to-date
            commit_status = misc_utils.compare_commit_hashes(local_commit_hash, latest_commit_hash)

            # Convert the last update time to a Unix timestamp
            last_update_timestamp = misc_utils.iso_to_unix_timestamp(last_update)
        ...
        * `drums`, `ds`, `d`:  Drums
        ...
        async def leaderboard(ctx, shortname: str = None, instrument: str = None, rank_or_account = None):
            ...
            matched_tracks = misc_utils.fuzzy_search_tracks(tracks, shortname)
            ...
            instrument_display_name, instrument_codename = misc_utils.get_instrument_from_query(instrument)
            if not instrument_codename:
                await ctx.send('Unknown instrument.')
                return

            leaderboard_entries = leaderboard_functions.fetch_leaderboard_of_track(matched_track['track']['sn'], instrument_codename)
            ...
            if len(leaderboard_entries) > 0:
                if not rank_or_account:
                    ...
                    embeds = leaderboard_functions.generate_leaderboard_entry_embeds(leaderboard_entries, title, chunk_size=10)
                    view = PaginatorView(embeds, ctx.author.id)
                    view.message = await ctx.send(embed=view.get_embed(), view=view)
                else:
                    ...
                    try:
                        ...
                        if entries:
                            await ctx.send(embed=leaderboard_functions.generate_leaderboard_embed(matched_track, entries[0], instrument_display_name))
                        else:
        ...
        @bot.command(
            name='setprefix', 
            help='Set a custom command prefix for your server.', 
            hidden=True
        )
        ...
        async def set_prefix(ctx, prefix: str):
            ...
            prefixes = session_utils.load_prefixes()
            guild_id = str(ctx.guild.id)
            prefixes[guild_id] = prefix
            session_utils.save_prefixes(prefixes)
        ...
        async def on_command_error(ctx, error):
            ...
            if isinstance(error, commands.MissingPermissions):
                await ctx.send("You do not have the necessary permissions to run this command. Only administrators can use this command.")
        ...
        async def check_for_new_songs():
            ...
            session_hash = session_utils.generate_session_hash(start_time, start_time)  # Unique session identifier
            ...
            tracks = misc_utils.fetch_jam_tracks_file()
            ...
            known_tracks = disk_utils.load_known_songs_from_disk()  # Reload known_tracks.json
            known_shortnames = disk_utils.load_known_songs_from_disk(shortnames=True)  # Reload known_songs.json

            disk_utils.save_known_songs_to_disk(tracks)
            ...
            for channel_id in CHANNEL_IDS:
                ...
                if new_songs:
                    ...
                    for new_song in new_songs:
                        embed = embed_utils.generate_track_embed(new_song, is_new=True)
                        await send_auto_publish_message(channel, embed)
                ...
                if removed_songs:
                    ...
                    for removed_song in removed_songs:
                        embed = embed_utils.generate_track_embed(removed_song, is_removed=True)
                        await send_auto_publish_message(channel, embed)
                        await asyncio.sleep(2)
                    disk_utils.save_known_songs_to_disk(tracks)

                if modified_songs and METADATA_COMPARING_ALLOWED:
                    ...
                    for old_song, new_song in modified_songs:
                        ...
                        local_midi_file_old = midi_utils.download_and_archive_midi_file(old_url, short_name)
                        local_midi_file_new = midi_utils.download_and_archive_midi_file(new_url, short_name)
                        ...
                        if (old_url != new_url) and DECRYPTION_ALLOWED and CHART_COMPARING_ALLOWED:
                            ...
                            await midi_utils.process_chart_url_change(local_midi_file_old, local_midi_file_new, channel, short_name, track_name, artist_name, album_art_url, last_modified_old, last_modified_new, session_hash)

                        embed = embed_utils.generate_modified_track_embed(old=old_song, new=new_song)
                        await send_auto_publish_message(channel, embed)
                        await asyncio.sleep(2)
                    disk_utils.save_known_songs_to_disk(tracks)
...
festival_info_bot = FestivalInfoBot()